...
{
    Style st_rectangle;
    st_rectangle.set(Color("#FFFFFF"), 1.0, Color("#FFFFFF"), 0.1, 1.0);
    ...
    return Polygon({p1, p2, p3, p4}, st_rectangle, "").getSvgVector();
}
...
vector<string> speedometer(Point origin, int size, int speed)
{
    ...
    if (speed > 45)
    {
        st_indicator.setFill(Color("#FF0000"), 1.0);
        st_indicator.setStroke(Color("#FF0000"), 1.0, 0.5);
    }
    else
    {
        st_indicator.setFill(Color("#00FF00"), 1.0);
        st_indicator.setStroke(Color("#00FF00"), 1.0, 0.5);         
    }

    temp = Circle(center.polar(radius[4], 45), radius[5] * 0.3, 
        st_indicator, "").getSvgVector();
    result.reserve(result.size() + temp.size());
    result.insert(result.end(), temp.begin(), temp.end());  
